class ProductsController < ApplicationController
  ...
  def create
    new_product = @merchant.products.create!(product_params)
    json_response(new_product, :created)
  end
  ...
  def set_product
    @product = @merchant.products.find(params[:id]) if @merchant
  end
end

# TODOs
# Return a custom success message after product creation/update/deletion
# Return the updated record after a PUT request, not empty contents
